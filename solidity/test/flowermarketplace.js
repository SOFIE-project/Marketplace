...
contract('FlowerMarketPlace', function (accounts) {
    ...
    it("testing requests & offers", async () => {
        ...
        let {quantity, flowerType} = await market.getRequestExtra(txx.logs[1].args.requestID.toNumber());
        assert.equal(quantity.toNumber(), 20, "quantity wasn't 20");
        assert.equal(flowerType.toNumber(), 3, "flowerType wasn't 3 (White)");
        let tx2 = await market.submitRequest(2000000000);
        await market.submitRequestArrayExtra(tx2.logs[1].args.requestID, [45, 0]);
        let tx3 = await market.submitRequest(2000000000);
        await market.submitRequestArrayExtra(tx3.logs[1].args.requestID, [100, 2]);
        ...
        let {status: statusx3, quantity: quantityx3, flowerType: flowerTypex3} = await market.getRequestExtra(3);
        assert.equal(statusx3.toNumber(), 0, "status wasn't successful");
        assert.equal(quantityx3.toNumber(), 100, "req3 quantity wasn't 100");
        assert.equal(flowerTypex3.toNumber(), 2, "req3 flowerType wasn't 2");
        ...
        txx = await market.submitOfferArrayExtra(tx1.logs[1].args.offerID, [100], {from: accounts[6]});
        assert.equal(txx.logs[0].args.status.toNumber(), 0, "status wasn't successful");
        ...
        assert.equal(price.toNumber(), 100, "price wasn't 100");
        tx2 = await market.submitOffer(3, {from: accounts[8]});
        await market.submitOfferArrayExtra(tx2.logs[1].args.offerID, [13], {from: accounts[8]});
        ...
        await market.submitOfferArrayExtra(tx4.logs[1].args.offerID, [593]);
        ...
        assert.equal(priceExt2.toNumber(), 13, "off2 price wasn't 13");
        ...
        assert.equal(accOffIDs[0].toNumber(), 3, "accepted offer ID wasn't 3 (666)");
