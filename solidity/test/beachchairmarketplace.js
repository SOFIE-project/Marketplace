...
contract('BeachChairMarketPlace', function(accounts) {
    ...
    it("testing owner & managers", function(done) {
        var market;
        BeachChairMarketPlace.deployed().then(function(instance) {
            market = instance;
    ...
    it("testing requests & offers", function(done) {
        ...
        BeachChairMarketPlace.deployed().then(function(instance) {
            ...
            return market.submitRequestArrayExtra(tx1.logs[1].args.requestID, [20, 20180809]);
        }).then(function(txx) {
            ...
            assert.equal(reqExtraInfo[2].toNumber(), 20180809, "date wasn't 20180809");
            return market.submitRequest(2000000000);
        }).then(function(tx2) {
            market.submitRequestArrayExtra(tx2.logs[1].args.requestID, [45, 20191111]);
        }).then(function() {
            return market.submitRequest(2000000000);
        }).then(function(tx3) {
            market.submitRequestArrayExtra(tx3.logs[1].args.requestID, [100, 20180927]);
        }).then(function() {
            ...
            assert.equal(ext3[2].toNumber(), 20180927, "req3 date wasn't 20180927");
        }).then(function() {
            ...
            return market.submitOfferArrayExtra(tx1.logs[1].args.offerID, [83, 1200], {from: accounts[6]});
        }).then(function(txx) {
            assert.equal(txx.logs[0].args.status.toNumber(), 0, "status wasn't successful");
        ...
        }).then(function(offerExtra) {
            assert.equal(offerExtra[1].toNumber(), 83, "quantity wasn't 83");
            assert.equal(offerExtra[2].toNumber(), 1200, "totalPrice wasn't 1200");
            return market.submitOffer(3, {from: accounts[8]});
        }).then(function(tx2) {
            market.submitOfferArrayExtra(tx2.logs[1].args.offerID, [13, 250], {from: accounts[8]});
        }).then(function() {
            return market.submitOffer(3, {from: accounts[2]});
        }).then(function(tx3) {
            market.submitOfferArrayExtra(tx3.logs[1].args.offerID, [25, 500], {from: accounts[2]});
        }).then(function() {
            return market.submitOffer(3);
        }).then(function(tx4) {
            market.submitOfferArrayExtra(tx4.logs[1].args.offerID, [85, 1234]);
        }).then(function() {
            return market.submitOffer(1, {from: accounts[6]});
        }).then(function(tx5) {
            market.submitOfferArrayExtra(tx5.logs[1].args.offerID, [34, 323], {from: accounts[6]});
        }).then(function() {
            ...
            assert.equal(ext2[1].toNumber(), 13, "off2 quantity wasn't 13");
            assert.equal(ext2[2].toNumber(), 250, "off2 totalPrice wasn't 250");
        }).then(function() {
            ...
            var accOfferIDs = [1, 2, 4];
            market.decideRequest(3, accOfferIDs);
        }).then(function() {
        ...
        }).then(function(accOffIDs) {
            assert.equal(accOffIDs[0].toNumber(), 0, "status wasn't successful");
            assert.equal(accOffIDs[1][0].toNumber(), 1, "accepted offer ID[0] wasn't 1");
            assert.equal(accOffIDs[1][1].toNumber(), 2, "accepted offer ID[1] wasn't 2");
            assert.equal(accOffIDs[1][2].toNumber(), 4, "accepted offer ID[2] wasn't 4");
        }).then(function() {
    ...
    it("testing status codes", function(done) {
        ...
        BeachChairMarketPlace.deployed().then(function(instance) {
            ...
            return market.submitOfferArrayExtra(res7.logs[1].args.offerID, [11, 111]);
        }).then(function(res8) {
            assert.equal(res8.logs[0].args.status.toNumber(), 0, "status wasn't successful");
            return market.submitOfferArrayExtra(res8.logs[1].args.offerID, [12, 112]);
        }).then(function(res9) {
            ...
            var accOfferIDs1 = [5, 5];
            return market.decideRequest(1, accOfferIDs1);
        }).then(function(res13) {
            assert.equal(res13.logs[0].args.status.toNumber(), 10, "status should've been improper list");
            var accOfferIDs2 = [4, 5];
            return market.decideRequest(1, accOfferIDs2);
        }).then(function(res14) {
            assert.equal(res14.logs[0].args.status.toNumber(), 10, "status should've been improper list");
            var accOfferIDs3 = [5, 6];
            return market.decideRequest(1, accOfferIDs3);
        }).then(function(res15) {
